...
public class LogMgr implements Iterable<BasicLogRecord> {
   ...
   public LogMgr(String logfile) {
      ...
      if (logsize == 0)
         ...
         System.out.println("Initially setting the current block in LogMgr: "+ currentblk);
   ...
   /**
    * Returns an forward iterator for the log records,
    * which will be returned in forward order starting with the most recent.
    * @see java.lang.Iterable#iterator()
    */
   public synchronized Iterator<BasicLogRecord> fwditerator() {
	      flush();
	      return new LogIterator(currentblk, true);
   }
